...
public class GeneralBasics2d : MonoBehaviour
{
		...
		private GameObject createSpriteDude(string name, Vector3 pos, bool hasParticles = true)
	{
		...
		if (hasParticles)
		{
			GameObject gameObject2 = Object.Instantiate<GameObject>(this.prefabParticles, Vector3.zero, this.prefabParticles.transform.rotation);
		...
		private void advancedExamples()
	{
		LeanTween.delayedCall(base.gameObject, 14f, delegate()
		{
			for (int i = 0; i < 10; i++)
			{
				...
				GameObject gameObject = this.createSpriteDude("dude" + i, new Vector3(-2.51208f, 7.100643f, -14.37754f), false);
				...
				float num2 = Mathf.Sin(num + LeanTween.tau * 0f / 3f) * 0.5f + 0.5f;
				float num3 = Mathf.Sin(num + LeanTween.tau * 1f / 3f) * 0.5f + 0.5f;
				float num4 = Mathf.Sin(num + LeanTween.tau * 2f / 3f) * 0.5f + 0.5f;
				Color to;
				to..ctor(num2, num3, num4);
